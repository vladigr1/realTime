...
int waitAll() {
	if(waitType[currpid] != 0){
		printf("this procces supose to wait but isnt\n");
		return 0;
	}
	if(sonNumaTable > 0){
		waitType[currpid] = WAIT_ALL;
		suspend(currpid);
	}
	return 0;
}
...
int waitID(int id) {
	...
	if(paTable[id] != currpid){
		printf("this procces doesnt have such a son\n");
		return 0;
	}
	if(sonNumaTable > 0){
		waitType[currpid] = id;
